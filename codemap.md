...
| `scripts/` | Repository-wide BrainPet contracts, six-target release assembly/aggregation, GitHub OIDC/Sigstore evidence signing and verification, trusted candidate and physical-evidence intake/download, source validation, and workspace release checks. | `scripts/brainpet-*.mjs` |
...
5. OpenPets loads the plugin service lazily on the first plugin or Control Center request, then persists plugin state/config, schedules declarative timers, and bridges SDK v3 calls through permission-checked host modules. BrainPet does not load or bundle the plugin platform; its training entry is a built-in feature. Voice input remains one-shot, visibly indicated, cancellable, timeout-bounded, and never ambient.
