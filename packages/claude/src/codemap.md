...
- **hooks.ts**: Hook execution engine. It maps stable Claude session ids onto the shared lifecycle contract (`working`, `waiting`, `ready`, `blocked`, `idle`) and sends exactly one `agent.activity`; automatic reaction/speech and lease acquisition are disabled.
