...
for (const target of brainPetReleaseTargets) {
  for (const mode of ["private-test", "public-release"]) {
    ...
    if (receipt.releaseTarget.id !== target.id || receipt.releaseMode !== mode || receipt.publicArtifact !== (mode === "public-release") || receipt.validatorAutomatic !== true) throw new Error(`Invalid package receipt for ${target.id}/${mode}`);
  }
