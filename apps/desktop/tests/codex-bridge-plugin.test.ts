...
test("Codex bridge plugin owns the full local task lifecycle", () => {
  ...
  assert.ok(hookDefinitions.every((hook) => hook.command.includes("bridge.sh")), "macOS hooks must route through the architecture-aware launcher.");
  ...
  assert.match(readFileSync(resolve(pluginRoot, "scripts/bridge.sh"), "utf8"), /macos-arm64\/brainpet-hook/);
