...
test("OpenPets remains unchanged and explicit test overrides are bounded", () => {
  ...
  assert.equal(resolveDesktopDistributionSettings("BrainPet", "openpets", "brainpet.exe", { packaged: true }).profile, "brainpet");
  ...
  assert.equal(isBrainPetFeatureEnabled(resolveDesktopDistributionSettings("OpenPets"), "1"), false);
  assert.equal(isBrainPetFeatureEnabled(resolveDesktopDistributionSettings("BrainPet"), "0"), false);
  assert.equal(resolveDistributionUpdateRepository(resolveDesktopDistributionSettings("BrainPet"), "attacker/fake", { packaged: true }), "hmhm2333/brainpet");
  assert.equal(resolveDistributionUpdateRepository(resolveDesktopDistributionSettings("OpenPets"), "developer/fork"), "developer/fork");
  assert.equal(resolveDistributionUpdateRepository(resolveDesktopDistributionSettings("OpenPets"), "https://example.com"), "alvinunreal/openpets");
});
