...
- `pnpm --filter @open-pets/desktop test:brainpet-electron`：通过 Windows 原生窗口热点（不调用 DOM `button.click()`）打开舞台，验证失焦暂停/恢复、640×360、Stage Exerciser、资源快照、renderer 崩溃隔离和按测试 profile 的进程清理。
...
当前体验测试优先使用 `scripts/brainpet-package-unpacked.mjs`。BrainPet distribution profile 不自动 seed OpenPets 的三个默认启用插件，空闲时保留 4 个 Electron 进程；普通 OpenPets profile 的默认插件行为不变。两个私测产物都未签名，正式分发仍需恢复发布元数据、代码签名和发布证书流程。
