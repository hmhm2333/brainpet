...
- 点击状态徽标展开紧贴宠物的像素活动窗格；再次点击收起，单条活动可独立移除。
...
实现状态（2026-08-14）：代码完成。主宠使用独立状态徽标与同窗口像素活动托盘，最多显示 5 条隐私最小活动；托盘展开时标记完成/失败结果已读。训练配件、状态徽标和宠物拖拽区使用独立事件目标。BrainPet 右键菜单已提供训练、五档缩放、暂停跟随和隐藏至下次活动，系统托盘可以从暂停跟随状态唤醒。Codex 当前只声明 `observeLifecycle`，因此没有伪造“停止/授权/回复/打开任务”按钮，这些继续留在 M4 的能力门控内。
